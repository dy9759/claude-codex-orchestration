...
if [ -f "$SKILL_DIR/scripts/run-with-timeout.sh" ]; then
  cp "$SKILL_DIR/scripts/run-with-timeout.sh" "$ORCH_DIR/bin/run-with-timeout.sh"
  chmod +x "$ORCH_DIR/bin/run-with-timeout.sh"
  echo "  -> bin/run-with-timeout.sh"
fi
