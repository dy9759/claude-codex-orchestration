...
10. **Capability routing** — task assignment by agent capability matrix (`references/runtime-routing.md`), not runtime identity. Bug fixes and detail changes in an existing project are **Codex-first** when scope is bounded and verifiable; exceptions: repo-wide architecture, frontend/UI judgment, cross-cutting refactors, or unclear ownership. High-risk ops (DB/env/secrets/package manifests/CI/release/destructive ops) are never auto-dispatched: current orchestrator writes a plan and asks explicit approval first. CC as runtime dispatches to Codex; Codex as runtime executes locally or dispatches to CC by the same matrix.
