...
11. **Heartbeat on dispatch** — every background cross-agent dispatch uses `dispatch-with-heartbeat.sh` for executable L1 process-alive / L2 progress / timeout monitoring. L3 semantic probe only counts when the orchestrator actually inspects partial output. Fallback on STALLED/DEAD/TIMEOUT per `references/heartbeat-protocol.md`.
